...
/**
 * Strip PII-shaped text from a query term (§19). Email addresses and phone
 * numbers never belong in a provider search — and must never reach stored
 * queries, cache scopes, or provider logs via generated search strings.
...
export function familyQuery(family, { withBoosters = 1 } = {}) {
  const pos = (family.positives || []).map((t) => sanitizeQueryTerm(t)).filter(Boolean).map((t) => `"${t}"`).join(' OR ');
  const boost = (family.boosters || []).slice(0, withBoosters).map((b) => sanitizeQueryTerm(b)).filter(Boolean).map((b) => `"${b}"`).join(' ');
